...
public class Christofides{
    ...
    public Christofides(boolean verbose, Point wind_direction){
    ...
    private double[][] buildWeightMatrix(double[] x, double[] y){
				...
				for(int u1=0;u1<dim;u1++){
						for(int u2=0;u2<dim;u2++){
								...
								Point p1 = new Point(x[u1], y[u1]);
								Point p2 = new Point(x[u2], y[u2]);
								dist = Point.getDistance(p1,p2);
								wt[u1][u2]=dist/Simulator.getSpeed(Point.getDirection(p1,p2), wind_direction);
								wt[u2][u1]=dist/Simulator.getSpeed(Point.getDirection(p2,p1), wind_direction);
						}
				}
